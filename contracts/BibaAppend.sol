...
///@title allws additional information/supporting material to be added to an existing evidence
contract BibaAppend{
   ...
   function returnTimes(bytes32 key) private view returns(uint){  // return number of times an evidence was appended to
      return TrackerMapping[key];
   }

   function append_allowed(bytes32 key, address ad_user, bytes32 key_of_new, uint case_num) private returns (bool){
